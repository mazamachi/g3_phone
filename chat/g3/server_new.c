...
void* thread2(void *arg)
{
  ...
  printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
  int fd2 = open(filename, O_RDONLY, 0644);
  while(read(fd2, buf, N) > 0) printf("%s", buf);
  printf("\n----------------------------------\n");
  printf("you > ");
  close(fd2);
  fwrite("\n\n", sizeof(char), 2, fd_chat);
  while( 1 ){
    gets(buf);
    ...
    fwrite("you > ", sizeof(char), 6, fd_chat);
    fwrite(buf, sizeof(char), strlen(buf), fd_chat);
    fwrite("\n", sizeof(char), 1, fd_chat);
  ...
  while(1) 
    {
      ...
      if(i == N) break;
      buf[x] ='\n';
      fwrite("your friend > ", sizeof(char), 14, fd_chat); 
      if(fwrite(buf, sizeof(char), x + 1, fd_chat) == -1) die("write1");
      sleep(1);
      fclose(fd_chat);
      printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
      fd2 = open(filename, O_RDONLY, 0644);
...
int main(int argc, char **argv) 
{
  ...
  printf("reject: r  accept: except 'r'\n ");
  char c = getchar();
  while(getchar() != '\n');
  if(c == 'r') return 0;
  char *str1 = malloc(sizeof(char) * N);
  strcpy(str1, ".chat");
  strcat(str1, ip_adress);
