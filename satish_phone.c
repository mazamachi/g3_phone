...
int main(int argc, char** argv)
{
 ...
 FILE *fp_rec;
 FILE *fp_play;
 if ( (fp_rec=popen("rec -t raw -b 16 -c 1 -e s -r 44100 -","r")) ==NULL) {die("popen:rec");}
 if ( (fp_play=popen("play -t raw -b 16 -c 1 -e s -r 44100 -","w")) ==NULL) {die("popen:play");}
  ...
  while(1){
    ...
    printf("\n%d \t %d \t %d \t %d \t %f\n",n_read, n_send, n_recv, n_write,((stop.tv_sec*1e6 + stop.tv_usec) - (start.tv_sec*1e6 + start.tv_usec)));
 }
